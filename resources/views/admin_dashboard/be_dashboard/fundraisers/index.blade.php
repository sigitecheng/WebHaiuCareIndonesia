    ...
    <div class="">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-slate-100 overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                @foreach($data_fundraiser as $data)
                <div class="item-card flex flex-row justify-between items-center">
                    <div class="flex flex-row items-center gap-x-3">
                        <img src="https://picsum.photos/200/300/?{{$data->user->nama}}" alt="https://picsum.photos/200/300/?{{$data->user->nama}}" class="rounded-2xl object-cover w-[80px] h-[60px]">
                    ...
                    <div  class="hidden md:flex flex-col">
                        ...
                        <p class="text-indigo-950 text-xl font-bold text-2xl">{{ $data->user->name}}</p>
                    </div>
                    <span class="font-bold py-1 px-6 bg-green-700 text-white rounded-full text-sm">
                        ACTIVE
                    </span>
                    <span class="font-bold py-1 px-6 bg-yellow-500 text-white rounded-full text-sm">
                        PENDING
                    </span>             
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <form action="#" method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="font-bold py-1 px-6 bg-indigo-700 text-white rounded-full text-sm">
                                Approve
                            </button>
                        </form>
                    </div>
                ...
                @endforeach
                            ...
                            <div class="list-fundraisers py-2">
                                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col ">
                                        <div class="item-card flex flex-col justify-between items-center gap-y-5">
                                            ...
                                            <form action="#" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="font-bold py-0 px-5 bg-indigo-700 text-white rounded-full">
                                                    Become Fundraiser
                                                </button>
                                            </form>
                                            <span class="w-fit text-sm font-bold py-1 px-3 rounded-full bg-orange-500 text-white">
                                                PENDING
                                            </span> 
                                        </div>
                                    </div>
